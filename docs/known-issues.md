...
- Phase RH-1 is merged; RH-2 special line/cross/wildcard cleansing merged (PR #10). **Rift Erosion Lab** remains the experimental hunger lab. **Thornwake Containment** (RH-3 draft) adds Puzzle Lab–only Fantasy pressure with playtest URL mode; human playtest pending. Calm campaign-style Fantasy levels remain threat-free. Boss encounters, campaign pressure wiring, special-created protection, cleanse score bonuses, and new objective kinds remain deferred (RH-4+). Coordinate-bound corruption does not block gravity. Cleansing does not reduce hunger and does not award score/collection. Automation baselines live in [rh3-pressure-balance-baseline.md](rh3-pressure-balance-baseline.md); they are not final balance proof. See [rift-hunger-board-threat-direction.md](rift-hunger-board-threat-direction.md).
